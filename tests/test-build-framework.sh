...
grep -q 'factory DATA image must not contain a Grok token' "$S2S_DATA_IMAGE" || fail "DATA image must assert Grok token absence"
grep -q 'factory DATA image must not contain a DashScope API key' "$S2S_DATA_IMAGE" || fail "DATA image must assert DashScope key absence"
grep -q 'factory DATA image must not contain a DashScope workspace ID' "$S2S_DATA_IMAGE" || fail "DATA image must assert DashScope workspace absence"
