...
grep -q '/data/swap' "$ROOT_DIR/saha-layers/meta-tegra-saha/recipes-saha/saha-data-layout/saha-data-layout/saha-data-layout.sh" || fail "DATA layout must create the protected swap directory"
SWAP_SERVICE="$ROOT_DIR/saha-layers/meta-tegra-saha/recipes-saha/saha-data-layout/saha-data-layout/saha-swap.service"
SWAP_SCRIPT="$ROOT_DIR/saha-layers/meta-tegra-saha/recipes-saha/saha-data-layout/saha-data-layout/saha-swap.sh"
grep -q 'SAHA_SWAP_SIZE_GIB=16' "$SWAP_SERVICE" || fail "Saha persistent swap must default to 16 GiB"
grep -q 'Requires=data.mount saha-data-layout.service' "$SWAP_SERVICE" || fail "Saha swap must require validated DATA"
grep -q 'swapon --priority 10' "$SWAP_SCRIPT" || fail "Saha swap must activate with explicit priority"
grep -q '^vm.swappiness=10' "$ROOT_DIR/saha-layers/meta-tegra-saha/recipes-saha/saha-data-layout/saha-data-layout/virtual-memory.conf" || fail "Saha swap must preserve physical memory with low swappiness"
...
OLLAMA_RECIPE="$ROOT_DIR/saha-layers/meta-tegra-saha/recipes-saha/ollama/ollama_0.32.5.bb"
OLLAMA_SERVICE="$ROOT_DIR/saha-layers/meta-tegra-saha/recipes-saha/ollama/ollama/ollama.service"
OLLAMA_MANAGER="$ROOT_DIR/saha-layers/meta-tegra-saha/recipes-saha/ollama/ollama/saha-ollama-manager.py"
[ -f "$OLLAMA_RECIPE" ] && [ -f "$OLLAMA_SERVICE" ] && [ -f "$OLLAMA_MANAGER" ] || fail "Ollama integration files must exist"
grep -q 'ollama' "$S2S_PACKAGEGROUP" || fail "S2S packagegroup must install Ollama"
grep -q 'OLLAMA_HOST=127.0.0.1:11434' "$OLLAMA_SERVICE" || fail "Ollama must remain loopback-only"
grep -q 'setpriv --reuid 10002 --regid 999 --groups 44,999' "$OLLAMA_SERVICE" || fail "Ollama must use the image's numeric unprivileged runtime identity"
grep -q 'OLLAMA_MODELS=/data/models/ollama' "$OLLAMA_SERVICE" || fail "Ollama models must use DATA"
grep -q 'OLLAMA_MAX_LOADED_MODELS=1' "$OLLAMA_SERVICE" || fail "Ollama must bound resident models on Orin NX"
...
grep -q 'model is not in the allowlisted catalog' "$OLLAMA_MANAGER" || fail "unified model manager must reject arbitrary model names"
grep -q '/v1/catalog' "$OLLAMA_MANAGER" || fail "unified model manager must expose the board-owned catalog"
grep -q '/v1/pipeline/readiness' "$OLLAMA_MANAGER" || fail "unified model manager must expose pipeline readiness"
grep -q '/v1/pipeline/mode' "$OLLAMA_MANAGER" || fail "unified model manager must expose pipeline mode switching"
grep -q 'https://api.roban.ai/v1' "$OLLAMA_MANAGER" || fail "Grok mode must use the fixed Roban endpoint"
grep -q 'grok-voice-latest' "$OLLAMA_MANAGER" || fail "Grok mode must use the fixed voice model"
grep -q 'qwen-audio-3.0-realtime-flash' "$OLLAMA_MANAGER" || fail "Qwen provider must use the approved realtime model"
grep -q 'cn-beijing' "$OLLAMA_MANAGER" || fail "Qwen provider must use the approved region"
grep -q 'balance_unavailable' "$OLLAMA_MANAGER" || fail "Grok must remain visible but unavailable"
! grep -Eq '(token|apiKey)[[:space:]]*=[[:space:]]*body' "$OLLAMA_MANAGER" || fail "manager API must not accept Grok credentials"
grep -q '/v1/network/status' "$OLLAMA_MANAGER" || fail "unified model manager must expose model-source network status"
grep -q 'ROBAN_S2S_SELECTION_PATH: /model-config/selection.json' "$S2S_COMPOSE" || fail "S2S must consume the unified manager selection"
! grep -Eq 'install -d -o 10002|-R 10002:999' "$S2S_MODELS_RECIPE" || fail "model package files must use build-known root ownership; DATA provisioning applies runtime ownership"
grep -q '/data/model-config/s2s:/model-config:ro' "$S2S_COMPOSE" || fail "S2S must mount unified model configuration read-only"
grep -q '/data/model-secrets/s2s:/model-secrets:ro' "$S2S_COMPOSE" || fail "S2S must mount its dedicated secrets directory read-only"
grep -q 'ROBAN_S2S_MODE_PATH: /model-config/pipeline-mode.json' "$S2S_COMPOSE" || fail "S2S must consume the board-owned mode config"
grep -q 'ROBAN_S2S_REALTIME_SETTINGS_PATH: /model-config/realtime-settings.json' "$S2S_COMPOSE" || fail "S2S must consume board-owned realtime settings"
grep -q 'qwen-audio-3.0-realtime-flash' "$S2S_DATA_IMAGE" || fail "DATA image must seed Qwen realtime defaults"
...
grep -q 'S2S_IMAGE ?= "roban-s2s:20260831-media-verification-fix-arm64"' "$S2S_IMAGE_RECIPE" || fail "S2S image tag must use the validated MCP URL fix contract"
...
grep -q '/health' "$S2S_COMPOSE" || fail "S2S container healthcheck must enforce liveness without requiring models"
...
! grep -Eq 'S2S_(STT|TTS)_(ARCHIVE|SHA256)' "$S2S_MODELS_RECIPE" || fail "factory DATA recipe must not seed STT or TTS models"
! grep -Eq 'dest/(stt|tts)|test_wavs/0.wav' "$S2S_MODELS_PREPARE" || fail "factory model preparation must retain only KWS/VAD"
grep -q 'root:root' "$S2S_MODELS_RECIPE" || fail "base speech package ownership must use identities known during Yocto QA"
grep -q 'manifest.sha256' "$S2S_MODELS_RECIPE" || fail "base speech resource package must generate a manifest"
grep -q 'base KWS/VAD verification failed' "$S2S_DATA_IMAGE" || fail "DATA image must verify packaged KWS/VAD resources"
grep -q 'factory DATA image must not contain a default STT model' "$S2S_DATA_IMAGE" || fail "DATA image must assert STT absence"
...
grep -q 'model-config/s2s/dashscope-workspace-id' "$S2S_DATA_IMAGE" || fail "DATA image must install optional DashScope workspace from .local-secrets"
grep -q 'file-checksums' "$S2S_DATA_IMAGE" || fail "DATA image must rebuild when .local-secrets credential files change"
grep -q 'model-secrets/s2s' "$S2S_DATA_IMAGE" || fail "DATA image must create the dedicated S2S secrets directory"
grep -q 'model-cache/s2s-model-manager' "$S2S_DATA_IMAGE" || fail "DATA image must create persistent manager state"
grep -q 'Restart=no' "$S2S_SERVICE" || fail "missing stage models must not create a systemd restart storm"
