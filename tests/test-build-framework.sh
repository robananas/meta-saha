...
grep -q '127.0.0.1:11434/v1' "$S2S_ENV" || fail "S2S must use Ollama OpenAI compatibility API"
...
grep -q 'ALLOWED' "$OLLAMA_MANAGER" || fail "Ollama manager must enforce an allowlist"
grep -q 'model is not in the allowlisted catalog' "$OLLAMA_MANAGER" || fail "Ollama manager must reject arbitrary model names"
grep -q '/data/model-config/s2s/ollama.env' "$S2S_LAUNCHER" || fail "S2S launcher must load the active Ollama model selection"
grep -q 'DEFAULT_CONFIG_PATH' "$OLLAMA_MANAGER" || fail "Ollama manager must expose the packaged default model before first activation"
