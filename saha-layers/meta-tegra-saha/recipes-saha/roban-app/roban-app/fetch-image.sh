...
try_local_tar() {
    ...
    if [ -n "$candidate" ] && [ -s "$candidate" ]; then
        ...
        echo "NOTE: Using validated local Roban workflow container archive: $candidate"
...
if [ -s "$dest_tar" ]; then
    if validate_archive "$dest_tar"; then
        echo "NOTE: Reusing validated Roban workflow container archive: $dest_tar"
        exit 0
    fi
    rm -f "$dest_tar"
fi
...
echo "ERROR: Validated local image ${image} or archive ${cache_name} is required for roban-workflow-api"
exit 1
