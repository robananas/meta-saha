...
saha_prepare_data_image() {
    ...
    test ! -e ${IMAGE_ROOTFS}/model-secrets/s2s/dashscope-api-key || bbfatal "factory DATA image must not contain a DashScope API key"
    test ! -e ${IMAGE_ROOTFS}/model-config/s2s/dashscope-workspace-id || bbfatal "factory DATA image must not contain a DashScope workspace ID"
